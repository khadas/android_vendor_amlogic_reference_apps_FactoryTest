...
public class WriteMacActivity extends Activity {
  ...
  private void OnScanText() {
    ...
    if (getResources().getInteger(R.integer.config_mac_length2) == nTextlen) {
      OnWriteMac(true);
    } else if (getResources().getInteger(R.integer.config_usid_length) ==
               nTextlen) {
    ...
    } else if (getResources().getInteger(R.integer.config_sn_length) ==
               nTextlen) {
      OnWriteSn();
    } else if (getResources().getInteger(R.integer.config_deviceid_length) ==
               nTextlen) {
      OnWriteDeviceid();
    } else {
      m_EditMac.setText("");
      ...
      //    		Toast.makeText(this, R.string.ScanError,
  ...
  public void OnWriteMac(boolean is_otp) {
    ...
    Log.d(TAG, "hlm WriteMac_ok_flag:" + WriteMac_ok_flag +
                   "WriteSn_ok_flag:" + WriteSn_ok_flag);
  ...
  public void Vim4WriteUsid() {
    ...
    if (strUsid.length() ==
        getResources().getInteger(R.integer.config_usid_length)) {
      for (int i = 0;
           i < getResources().getInteger(R.integer.config_usid_length); i++) {
        int value = (int)strUsid.charAt(i);
        if (((value > 0x2f) && (value < 0x3a)) ||
            ((value > 0x40) && (value < 0x47)) ||
    ...
    if (!format_err) {
      ...
      for (int j = 0;
           j < getResources().getInteger(R.integer.config_usid_length); j++) {
      ...
      if (MainActivity.burn_efuse_flag)
        Tools.writeFile(Tools.Efuse_Key_OTP_Usid, strUsid);
      else
        Tools.writeFile(Tools.Key_OTP_Usid, strUsid);
    }
    WriteSn_ok_flag = Tools.getUsid().contains(strUsid);
    ShowUsid();
  ...
  public void WriteMac(String strMac) {
    ...
    if (getResources().getBoolean(R.bool.config_write_mac_in_otp)) {
      if (strMac.length() == 17) {
        if ((':' == strMac.charAt(2)) && (':' == strMac.charAt(5)) &&
            (':' == strMac.charAt(8)) && (':' == strMac.charAt(11)) &&
            (':' == strMac.charAt(14))) {
          ...
          if (!format_err) {
            if (MainActivity.test_board.equals("VIM4")) {
              if (MainActivity.burn_efuse_flag)
                Tools.writeFile(Tools.Efuse_Key_OTP_Mac, mac);
              else
                Tools.writeFile(Tools.Key_OTP_Mac, mac);
              WriteMac_ok_flag = Tools.getEthMac().contains(strMac);
            } else {
              String cmd = String.format(
                  "setbootenv ubootenv.var.factory_mac %s", strMac);
  ...
  public void WriteUsid(String strUsid) {
  ...
  public void ShowMac_OTP() {
    if (MainActivity.test_board.equals("VIM4") ||
      ...
      if (length != getResources().getInteger(R.integer.config_mac_length)) {
        ...
        //	strTmpMac += strMac.substring(i, (i + 2) < length ? (i + 2) :
        // length ); 	if( (i + 2) < length) strTmpMac += ':';
        // }
  ...
  public void ShowUsid() {
    ...
    } else if (MainActivity.test_board.equals("VIM1S")) {
      if (Tools.getUsid().equals("00000000000000")) {
        m_UsidAddr.setTextColor(Color.RED);
      } else {
        m_UsidAddr.setTextColor(Color.GREEN);
      }
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    m_MacAddr_Title.setText(
        m_MacAddr_Title.getText().toString() + "\t\t\t" +
        getResources().getInteger(R.integer.config_mac_length) +
        getResources().getString(R.string.showLength));

    m_SnAddr_Title = (TextView)findViewById(R.id.TextView_sn_title);
    m_SnAddr_Title.setText(
        m_SnAddr_Title.getText().toString() + "\t\t\t" +
        getResources().getInteger(R.integer.config_sn_length) +
        getResources().getString(R.string.showLength));

    m_UsidAddr_Title = (TextView)findViewById(R.id.TextView_usid_title);
    m_UsidAddr_Title.setText(
        m_UsidAddr_Title.getText().toString() + "\t\t\t" +
        getResources().getInteger(R.integer.config_usid_length) +
        getResources().getString(R.string.showLength));

    m_DeviceidAddr_Title = (TextView)findViewById(R.id.TextView_deviceid_title);
    m_DeviceidAddr_Title.setText(
        m_DeviceidAddr_Title.getText().toString() + "\t\t\t" +
        getResources().getInteger(R.integer.config_deviceid_length) +
        getResources().getString(R.string.showLength));
