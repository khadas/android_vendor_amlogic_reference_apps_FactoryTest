...
public class PointerLocationView extends View implements OnTouchListener {
  ...
  protected void onDraw(Canvas canvas) {
    synchronized (mPointers) {
      ...
      canvas.drawText("Current Point:" + mCurNumPointers, getWidth() / 2,
      ...
      for (int p = 0; p < NP; p++) {
        ...
        if (mCurDown && ps.mCurDown) {
          canvas.drawLine(0, (int)ps.mCurY, getWidth(), (int)ps.mCurY,
                          mTargetPaint);
          canvas.drawLine((int)ps.mCurX, 0, (int)ps.mCurX, getHeight(),
                          mTargetPaint);
  ...
  public void addTouchEvent(MotionEvent event) {
    synchronized (mPointers) {
      ...
      for (int i = 0; i < NI; i++) {
        ...
        if (mPrintCoords) {
          Log.i("Pointer", "Pointer " + (id + 1) + ": (" + event.getX(i) +
                               ", " + event.getY(i) + ")"
